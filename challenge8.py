...
def do_challenge_b(lines):
    ...
    for distinct_letter in distinct_letters:
        ...
        for sub_comb in sub_combs:
            ...
            if p1_x == p2_x:
                y_dist = abs(p1_y - p2_y)
                if p1_y < p2_y:
                    new_p1_y = p1_y - y_dist
                    while new_p1_y >= min_y:
                        antinodes.add((p1_x, new_p1_y))
                        new_p1_y = new_p1_y - y_dist
                    new_p2_y = p2_y + y_dist
                    while new_p2_y <= max_y:
                        antinodes.add((p1_x, new_p2_y))
                        new_p2_y = new_p2_y + y_dist
                else:
                    new_p2_y = p2_y - y_dist
                    while new_p2_y >= min_y:
                        antinodes.add((p1_x, new_p2_y))
                        new_p2_y = new_p2_y - y_dist
                    new_p1_y = p1_y + y_dist
                    while new_p1_y <= max_y:
                        antinodes.add((p1_x, new_p1_y))
                        new_p1_y = new_p1_y + y_dist
            elif p1_y == p2_y:
                x_dist = abs(p1_x - p2_x)
                if p1_x < p2_x:
                    new_p1_x = p1_x - x_dist
                    while new_p1_x >= min_x:
                        antinodes.add((new_p1_x, p1_y))
                        new_p1_x = new_p1_x - x_dist
                    new_p2_x = p2_x + x_dist
                    while new_p2_x <= max_x:
                        antinodes.add((new_p2_x, p1_y))
                        new_p2_x = new_p2_x + x_dist
                else:
                    new_p2_x = p2_y - x_dist
                    while new_p2_x >= min_x:
                        antinodes.add((new_p2_x, p1_y))
                        new_p2_x = new_p2_x - x_dist
                    new_p1_x = p1_y + x_dist
                    while new_p1_x <= max_x:
                        antinodes.add((new_p1_x, p1_y))
                        new_p1_x = new_p1_x + x_dist
            elif p1_x < p2_x:
                x_dist = abs(p1_x - p2_x)
                ...
                if p1_y < p2_y:
                    new_p1_x = p1_x - x_dist
                    new_p1_y = p1_y - y_dist
                    while min_x <= new_p1_x <= max_x and min_y <= new_p1_y <= max_y:
                        antinodes.add((new_p1_x, new_p1_y))
                        new_p1_x = new_p1_x - x_dist
                        new_p1_y = new_p1_y - y_dist
                    new_p2_x = p2_x + x_dist
                    new_p2_y = p2_y + y_dist
                    while min_x <= new_p2_x <= max_x and min_y <= new_p2_y <= max_y:
                        antinodes.add((new_p2_x, new_p2_y))
                        new_p2_x = new_p2_x + x_dist
                        new_p2_y = new_p2_y + y_dist
                else:
                    new_p1_x = p1_x - x_dist
    ...
    antinodes.update(set([(letter.x, letter.y) for letter in flattened]))
    return len(antinodes)
